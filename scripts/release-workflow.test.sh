...
expect_equal(jobs.fetch("build-sign-notarize").fetch("environment"), "npm-production", "release approval environment")
...
telemetry_guard_index = build_steps.index { |step| step["name"] == "Guard telemetry webhook" }
goreleaser_index = build_steps.index { |step| step["name"] == "Run GoReleaser" }
contract_error("telemetry webhook must be checked before GoReleaser") unless telemetry_guard_index && goreleaser_index && telemetry_guard_index < goreleaser_index
contract_error("GoReleaser must receive the telemetry webhook") unless build_steps.fetch(goreleaser_index).fetch("env").key?("YOP_TELEMETRY_WEBHOOK")
...
macos_verify_step = macos.fetch("steps").find { |step| step["name"] == "Verify macOS binary" }
...
contract_error("verify-macos must verify the archive checksum") unless macos_verify_run&.include?("shasum -a 256 -c")
...
expect_equal(macos_notarize.fetch("ids"), ["yop-cli"], "notarized build IDs")
